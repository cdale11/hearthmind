...
page, following directly off A12.6-A12.8 (v1.34.287). Closes A4.3
(human rating / calibration ground truth) — the blind-pairwise data
model (`HumanRatingTask`/`HumanRating`, append-only JSONL, `judge_
human_agreement`, `hearthbench/scoring/human.py`) was already real and
untouched; only a page/routes consuming it were missing. **Closes A12
in full**, and closes Phase 6's own last gap within steps 6/7 — only
`A5.11` (step 8, gated on a live LLM server this offline environment
lacks) and the never-blocking `A6`/`A7.2`/`A11.1`-`A11.3`/`A11.5`/
A9.1's graphs remain open in Phase 6.
...
run's completion text lands in slot "a" vs. "b" is derived from that
same hash rather than a fixed order; the JSON response never includes
`candidate_a_source`/`candidate_b_source`/either judge score — A4.3's
own "must not be surfaced to the rater" holds at the HTTP boundary,
not only in `page.py`'s rendering. `POST /api/rating/submit` appends a
...
pair's own task ids only. New `_judge_value(record)` reads a case's
real Tier 2 judge composite off its `scores` dict (any scorer id
starting with `"judge"`) when one exists, `None` otherwise.
...
run_id on both new GET routes. All pass, first run — one genuine
pre-existing flake was caught and confirmed harmless along the way: a
single-shot (non-retrying) progress check in the unmodified A12.3/
A12.4 slow-backend section raised `KeyError` on a rare early-poll
race; two further clean re-runs (63/63, 0 failures, twice) confirmed
it as a known pre-existing timing flake in code this pass never
touched, not a regression.

Verified: the extended script (63 checks, 3 consecutive clean runs);
`node --check` on the page's own embedded JS (extracted from the
actual evaluated Python string this time, not the raw source text —
the raw source's own backslash-escaping produced a false-positive
syntax error on the first attempt); `pyflakes` clean on all three
touched files; `verify_hearthbench_isolation.py`/`verify_hearthbench_
adapter_isolation.py`/`verify_a1_3_process_isolation.py`/`verify_a7_
a8_run_diagnostics.py`/`verify_a9_a10_score_report.py`/`verify_a13_ci_
guard.py`/`verify_c5_model_passport.py`/`verify_a5_1_6_subjective_
categories.py`/`verify_a5_categories.py`/`verify_a4_scoring.py` (incl.
its own pre-existing `HumanRatingTask` round-trip check) all re-run
clean. No `simulation/engine.py` code path or native module touched
(confirmed via `git status` — only `hearthbench/daemon/` and the
verify script changed) — no replay-hash/native-soak re-run needed.
...
— all six A5.1-A5.6 subjective benchmark categories are now real,
built on a generalized A4.2 `JudgeScorer`. **This closes step 7 of the
checklist's own SEQUENCE in full** — only A4.3's human-rating page UI
(deferred to A12.9) remains open within either item.

`hearthbench/scoring/judge.py`'s `JudgeScorer` previously hardcoded
ONE rubric (dialogue's naturalness/personality/emotional-realism) via
module-level constants — the real blocker every one of A5.2-A5.6's own
distinct judged axes needed removed first. Gained optional `rubric_
prompt`/`axes`/`rubric_version` constructor params, each defaulting to
the pre-existing module constants; `JudgeScorer(adapter)` with no
extra args reproduces the exact original dialogue scorer byte-for-byte
(verified directly), zero behavior change for any pre-existing call
site. `build_judge_prompt()` gained a matching optional `rubric_
prompt` param with the same default-preserving contract; `as_scorer()`
gained an optional `description` override.

Six new `hearthbench/tests/` modules, each following `grounding.py`'s
own shape (a real `Category` at A10.1's own stated weight, real hand-
authored `TestCase`s, a `build_*_judge_scorer(adapter)` factory):
`dialogue.py` (A5.1, weight 15 — reuses the DEFAULT rubric unmodified,
since its own checklist text is exactly what that rubric already
scores; new Tier 1 scorer `no_ambient_filler`, a closed vocabulary of
generic-agreement/aphorism filler phrases — the checklist's own
explicitly-named gap, distinct from `VOICE_LINE_DUPLICATE_OVERLAP`/
`FOLKLORE_DUPLICATE_OVERLAP`'s exact-repeat detection); `personality.py`
(A5.2, weight 10 — voice consistency/distinctiveness/trait
plausibility; honest scope trim in its own docstring: scores ONE
case's output against a stated profile, not cross-conversation
aggregation, which needs a future A11 runner); `memory.py` (A5.3,
weight 12 — recall accuracy/appropriate forgetting/contradiction
resistance, the first real category built on A3.3's own `Turn.
injected_fact`/`expects_recall_of`/`offers_contradiction` machinery);
`beliefs.py` (A5.4, weight 12 — evidence grounding/revision quality/
confidence calibration; `revision_on_new_evidence` is the deliberate
mirror image of Memory's `contradiction_resistance` case — same `Turn`
machinery, opposite correct behavior: Memory tests HOLDING to a known
truth against a false contradiction, Beliefs tests REVISING a theory
when the evidence genuinely changes); `planning.py` (A5.5, weight 8 —
...
SETTLEMENT-scale collective "village voice," matching how `llm/town_
brain.py`/`llm/beliefs.py`'s settlement-scoped path/`llm/culture.py`
already speak in production, never a single named agent). Every
non-dialogue rubric prompt/axes tuple is verified pairwise-distinct,
not just individually different from dialogue's default.
...
`no_ambient_filler` alongside `no_unsupported_specifics`. Every
`judge_*` scorer is deliberately NOT auto-registered (per `hearthbench.
scoring`'s own stated Tier 2/3 discipline — each needs a live adapter
at construction time); `build_*_judge_scorer(adapter)` is the real
per-category wiring a caller uses instead. `hearthbench/reporting/
score.py`'s `MISSING_SUBJECTIVE_CATEGORY_WEIGHTS` is now genuinely
empty (every category it recorded is real) — kept, not deleted, as the
real extension point it always was for a future genuinely-new
category.
...
adapter): `JudgeScorer`'s backward compatibility; all nine `CATEGORY_
REGISTRY` weights matching A10.1's table exactly; every new category's
real cases; all six rubric prompts pairwise-distinct; `no_ambient_
filler`'s clean/filler-laden/heavily-filler-laden/empty cases; Memory's
and Beliefs' real multi-turn machinery incl. the deliberate final-turn
`expects_recall_of` distinction between the two categories' cases; two
full real end-to-end judge round-trips (Memory, Personality) confirming
the real HTTP request carries each category's OWN rubric/axes, never
Dialogue's; `measure_self_consistency` composing cleanly with a custom
(Planning) rubric. All pass, first run, no bug found in the module
under test.
...
`pyflakes` clean on all new/touched files; `verify_a4_scoring.py`/
`verify_a5_categories.py` (updated for the real 9-category `CATEGORY_
REGISTRY`)/`verify_a9_a10_score_report.py`/`verify_a13_ci_guard.py`/
`verify_a7_a8_run_diagnostics.py`/`verify_a12_bench_daemon.py`/
`verify_a1_3_process_isolation.py`/`verify_c5_model_passport.py` all
re-run clean. No native module or `simulation/engine.py` code path
touched (confirmed via `git status`) — only `hearthbench/` package
files, `docs/`, and `scripts/verify_*.py` changed — no replay-hash/
...
isolation), the prerequisite A12 (still open) itself names: "Bench
runs execute in a subprocess with their own model server config...
The UI page talks to a bench daemon, not the sim engine." A2/A11 were
already real, so this was pure process-boundary wiring.

New `hearthbench/runner/cli.py`: a real `python -m hearthbench.
runner.cli run` entry point — resolves a category to real `TestCase`s
(`_cases_for_category`, today `"grounding"` only), builds a real
`OpenAICompatAdapter` + A8 environment snapshot, drives `run_cases_
with_resume` — orchestration only, zero new execution logic. New
`hearthbench/runner/process.py`'s `BenchRunProcess` mirrors A2.4's
`ServerLifecycle` (same `LaunchRecord`, reused not duplicated) but
purpose-built for a bench run: `poll_progress` reads real progress
purely off disk via A8's `RunRecordReader.completed_case_ids()` — no
IPC beyond the filesystem both processes already share; a nonzero
exit with incomplete work is flagged `crashed: True`.

**Real bug found and fixed via the crash test, not in this pass's own
new code.** Forcing a genuine crash (a plain file where a run_dir
should be, so `RunRecordWriter.__init__`'s `mkdir()` raises inside the
child) worked — but polling that broken path afterward via `RunRecord
Reader` itself then crashed: `BlobStore.__init__` (shipped v1.34.281)
unconditionally called `mkdir()` even for a read-only reader, and
`<file>/blobs` can't be created as a subdirectory of a plain file.
Fixed by making `BlobStore.__init__` touch no filesystem state at all
— only `put()` (a real write) ever creates a directory now; `get()`
gained a `try/except OSError` degrade. Exactly the class of gap real
non-mocked subprocess verification exists to catch.

New `scripts/verify_a1_3_process_isolation.py` (27 checks, all pass
after the fix above — real local HTTP server, real `subprocess.Popen`,
never mocked): pure-function coverage; `cli.main()` in-process against
a real fake server; `BenchRunProcess` as a genuine OS subprocess —
double-start rejection, real mid-run partial progress read purely off
disk while a deliberately slow fake server is still answering, clean
completion (4/4 committed, never flagged crashed), the real crash case
above, and a real `stop()` against a genuinely hung 30s request.

Verified: the new script (27 checks); `verify_hearthbench_
isolation.py` (39 files)/`verify_hearthbench_adapter_isolation.py`
both clean; `pyflakes` clean; `verify_a2_model_adapters.py`/`verify_a3_
prompt_library.py`/`verify_a4_scoring.py`/`verify_a5_categories.py`/
`verify_a13_ci_guard.py`/`verify_a7_a8_run_diagnostics.py`/`verify_a9_
a10_score_report.py` all re-run clean. No native module or `simulation/
engine.py` code path touched — `git status` confirmed only `hearthbench/
diagnostics/run_record.py` (the fix), `hearthbench/runner/__init__.py`,
the two new `hearthbench/runner/` modules, and the new verify script
changed. Per SEQUENCE, `A12` (now genuinely unblocked) and `C5` (model
...
regression guard) plus the minimal real A11 execution-core slice A13
depends on. Re-reading the checklist's own SEQUENCE found a real
self-correction needed first: it places A13 right after "A4.1
deterministic scorers + A5.7/A5.8" (what A4/A5 just shipped),
explicitly BEFORE A6-A12 — this doc's own roadmap had wrongly deferred
A13 to the very end from an earlier pass; corrected, and A13 ships now.

New `hearthbench/runner/run.py` (A11's real slice, not the full mode
abstraction — quick/full/custom/resume/strict-repro remain unbuilt):
`render_case_prompt` resolves a `TestCase` into an executable prompt
via `turns` (A3.3's `render_turn_sequence`, last turn's content) or a
`fixture_ref` (A3.1's `FixtureExample`s); neither path fabricates a
prompt — an unresolvable case returns `None`, treated as a genuine
skip. `run_case_against_adapter`/`run_cases_against_adapter` are the
real call-and-score loop; `aggregate_scores`/`summaries_to_metrics_
dict` reshape per-case `ScoreDetail`s into the dotted-path dict
`eval_harness.check_regressions` consumes.

New `hearthbench/reporting/ci_guard.py`: A13.1 `is_relevant_change`
(any file under `hearthmind/llm/`/`hearthbench/prompts/`/
`hearthbench/scoring/` triggers, maintenance-free); A13.2 `DEFAULT_CI_
THRESHOLDS` (objective-only — grounding/structured-outputs, never
judge-scored); A13.3 `run_ci_guard` reuses `eval_harness.check_
regressions` DIRECTLY (real A0.2 reuse); A13.4 `save_baseline`/`load_
baseline` (explicit refresh only, missing file degrades to `{}`).
A13.5 (real nightly CI wiring) stays unbuilt — needs real CI infra
this repo doesn't have.

**Exercising the real end-to-end path for the first time found and
fixed a real, previously-shipped bug.** A5.7's `no_unsupported_
specifics` scorer was correctly declared on every grounding `TestCase`
but had never been `.register()`-ed into `hearthbench.scoring.
DEFAULT_REGISTRY` — `verify_a5_categories.py` called it directly,
bypassing the registry, so this silent gap survived until this pass's
`registry.resolve(case.scorers)` call (which drops an unknown id
silently rather than raising) finally exercised it. Fixed with one
line in `hearthbench/tests/__init__.py` (the only legal non-circular
place); `verify_a5_categories.py` re-run unmodified afterward, still
47/47.

New `scripts/verify_a13_ci_guard.py` (33 checks, all pass — the fix
above found via this script, not a bug in the new module): real local
HTTP server standing in for a model backend through the real
`OpenAICompatAdapter`; trigger logic; all three `render_case_prompt`
paths; skip-makes-zero-HTTP-calls; the full `run_ci_guard` path
against three real fake-server personas (no-op/clean-pass/caught-
fabrication-violation); baseline save/load round-trip; a custom
threshold dict genuinely overriding the default.

Verified: the new script; `scripts/verify_hearthbench_isolation.py`/
`verify_hearthbench_adapter_isolation.py` both clean; `pyflakes`
clean; `verify_a2_model_adapters.py`/`verify_a3_prompt_library.py`/
`verify_a4_scoring.py`/`verify_a5_categories.py` all re-run clean. No
native module, `simulation/engine.py` code path, or other production
file touched — no replay-hash/native-soak re-run needed. Per the
checklist's own SEQUENCE, A6 (structured-output validator) is the
next unstarted dependency-ordered item — resume only on future
explicit direction.

## Current state (v1.34.279)
...
plus A5.10's extension guide, following A4 (v1.34.278). Per the
checklist's own SEQUENCE, ships exactly what's buildable on A4.1's
real Tier 1 scorers alone (no judge model needed); the six subjective
categories (A5.1-A5.6) stay explicitly open.

New `hearthbench/tests/category.py`: `Category` (weight mirrors
A10.1's own stated default composite table) + `summarize_scores`/
`CategoryScoreSummary` — a real, tested, category-scoped slice of
A7.3's stated statistics (N/mean/median/p95/stdev/95% CI), explicitly
NOT the full A7 metrics collector (no per-run record, no system-
sampling thread), flagged as such rather than overclaimed.

**A5.7 Grounding** (`grounding.py`, weight 20): four real hand-
authored adversarial `TestCase`s, each baiting a specific fact its own
`structured_input` deliberately withholds. New category-specific
scorer `no_unsupported_specifics`: flags a number/proper-noun claim in
the output with no support in `structured_input`, bounded so heavy
fabrication saturates rather than falling unboundedly. "Reward
explicit uncertainty" needed no separate logic — a hedge introduces no
new specifics, so it already scores clean under the same heuristic.

**A5.8 Structured outputs** (`structured_outputs.py`, weight 8):
`build_structured_output_cases()` derives a real `(grammar, no_
grammar)` case pair per task actually registered in `hearthmind.llm.
json_schemas.TASK_SCHEMAS` — zero hardcoded task list. `score_
structured_output_delta` answers A6.3's identical "score both modes,
report the delta" using only `CategoryScoreSummary`'s `pass_rate`.

**A5.9 Performance** (`performance.py`, weight 5): wraps A4.1's
`latency` scorer; `summarize_latency` computes real p50/p95/max/mean
for latency, TTFT, and completion tok/s. Continuous RAM/swap/CPU%
sampling (A7.2) stays unstarted future work.

**A5.10**: `docs/HEARTHBENCH-CATEGORY-GUIDE.md` (a real 5-step
walkthrough) + `hearthbench/tests/_template.py` (a real, importable,
deliberately unregistered starter module).

New `scripts/verify_a5_categories.py` (47 checks, all pass — one real
test-DATA fix, not a module bug: a bait-question substring check
looked for "population," never literally in the real rendered
question — fixed to check for "marshcroft").

Verified: the new script; `scripts/verify_hearthbench_isolation.py`/
`verify_hearthbench_adapter_isolation.py` both clean (31 hearthbench
files); `pyflakes` clean; `verify_a2_model_adapters.py`/`verify_a3_
prompt_library.py`/`verify_a4_scoring.py` all re-run clean. No native
module, `simulation/engine.py` code path, or other production file
touched (confirmed via `git status`) — no replay-hash/native-soak
re-run needed. The six subjective A5 categories (needing A4.2's judge
tier + real content authoring) are the next dependency-ordered Phase 6
item — resume only on future explicit direction.
...
(HearthBench itself, `docs/ROADMAP-2026-07-REMAINING.md`'s
dependency-ordered A2→A3→A4→A5→A6→A7/A8/A9→A10→A11→A1.3→A12→A13),
shipping A2 (Model Adapter Layer) — the first item every later
HearthBench piece is built against. Only A0/A1.1/A1.2 existed before
this pass.

A2.1: `hearthbench/adapters/protocol.py`'s `ModelAdapter` — a
`typing.Protocol` (structural typing, no shared inheritance) +
`AdapterResult`/`AdapterCapabilities`/`AdapterDescribe`/`HealthStatus`
dataclasses, matching the checklist's own literal shape. Honest `seed`
support needed one small additive change to a live production file:
`hearthmind.llm.client`'s `OllamaClient`/`LlamaCppClient.generate_json`
gained a trailing `seed_override` param, mirroring the existing `num_
predict_override`/`temperature_override` pattern exactly — appended
last, `None` reproduces every existing positional call site byte-for-
byte. Without it the wrapping adapters would have had to falsely
report `seed=False` for a capability both backends' wire protocols
genuinely have, purely to avoid a second HTTP implementation — A0.1's
own "reuse, don't rebuild" applied one layer deeper.

A2.2: `LlamaCppAdapter`/`OllamaAdapter` thin-wrap `hearthmind.llm.
client`'s existing clients (real reuse — that module has no
`hearthmind.simulation`/`.agents`/`.world` dependency, so wrapping it
stays inside A1.2's isolation firewall); `OpenAICompatAdapter` is
genuinely new, self-contained, model-family-agnostic. New
`hearthbench/adapters/registry.py`'s `build_adapter`. The checklist's
own "enforce with a lint rule banning model-name string comparisons
outside `adapters/`" shipped as `scripts/verify_hearthbench_adapter_
isolation.py` (AST scan over comparison operands, clean on the real
tree). A2.3: `conformance.py`'s `run_conformance_suite` — adapter-
shape-agnostic, verified against a real local stdlib HTTP server
(success path, a genuine HTTP round-trip) and a real unreachable host
(failure path, genuine connection-refused, never a mock), plus a
deliberately-broken synthetic adapter proving the suite genuinely
catches a real contract violation. A2.4: `lifecycle.py`'s `build_
llama_server_command` (pure, mirrors `scripts/run.sh`'s own confirmed
defaults) + `ServerLifecycle` (generic subprocess wrapper, verified
against a real subprocess since no `llama-server` binary exists in
this offline environment).

New `scripts/verify_a2_model_adapters.py` (52 checks, all pass first
run). Verified: the new script; `scripts/verify_hearthbench_
...
MATCH (both re-run since `hearthmind/llm/client.py` was touched).
A3 (prompt library/test definitions) is the next dependency-ordered
Phase 6 item — resume only on future explicit direction.
...
the three tractable B15 sub-items in one batch; B11/B12/B14.3 all need
real new-consumer/new-mechanism design and stay explicitly open.

**B15.6.** New `World.machine_profile_history` (bounded 100, persisted
through `to_dict`/`from_dict`, legacy-backfilled): B15.2's own
"[DECIDED]" text names the exact gap this closes — "save files must
record the profile." A `session_started` entry (real `host_
fingerprint()`) per real `SimulationEngine` construction, plus
`rung5_entered`/`rung5_exited` on a genuine `Rung.REDUCE_COGNITION_
BREADTH` transition inside `_maybe_advance_escalation_ladder`'s own
resolver — B15.4's own "the one rung that does real, visible work,"
the one worth a durable cross-session record. New `machine_profile_
history_recent` diagnostics key, sibling to the runtime-only
`escalation_ladder.history_recent`.
...
runtime decision" claim carefully (recorded in the script's own
docstring — easy to get wrong): NOT that different runtime configs
must produce the SAME world state (B15.2's own "adaptive" half
explicitly allows Mind-layer content to differ by hardware — "the
same seed on different hardware produces different stories" is an
accepted consequence, not a bug). What must hold, and what this fuzzes,
is the narrower claim `verify_replay_hash.py` already proves for ONE
fixed config: a GIVEN randomized config must still reproduce byte-
identically across two independent runs of ITSELF. Same two-
independent-subprocess-runs technique, looped over K randomized
configs (`llm_max_concurrent`/`snapshot_every_ticks`/forced `dormancy_
aggressiveness` via `_last_strategy`) instead of the one default — 8
trials confirmed clean.
...
hypothesis-APPLY-time equivalence gate: an out-of-range starting
`value` is rejected outright (previously would have persisted
unclamped indefinitely — `Tunable.clamp()` was only ever consulted by
`adjust`/`set_value`), and a `SafetyClass.SENSITIVE` tunable with no
`description` is rejected — the "constants need a one-line docstring
explaining why" discipline made structural for exactly the tunables
risky enough to need B13.2's proof. Four test-fixture `Tunable(...)`
calls across three verify scripts needed a description added.

New `scripts/verify_b15_6_and_b15_8.py` (20 checks). **This closes
Tier 5 B15 in full** — B15.1 (v1.34.102) through B15.8 (this pass) are
all real.

Verified: both new scripts; `scripts/verify_tuning.py`/`verify_
optimization_hypothesis.py`/`verify_tunable_evolution.py`/`verify_
persistence_scheduling.py`/`verify_b15_escalation_ladder.py`/`verify_
b13_5_pacing_genome_evolution.py`/`verify_h2_h3_runtime_domain.py`/
`verify_b13_dev_console_endpoint.py`/`verify_runtime_diagnostics.py`/
`verify_scheduler.py`/`verify_task_graph.py`/`verify_dormancy.py`/
`verify_runtime_invariant.py`/`verify_b0_runtime_migrations.py` all
...
script. Also how long to wait for recorder to start training, should
I stop the world while training?" Closes Phase 2's last two items
(L2.1, L4.1) using the "trains directly from a live world's own db,
no recorder archive needed" pattern `train_embedding_from_world.py`
already established — both real data sources already live inside a
running world, not the LLM recorder archive.

L2.1: `hearthmind/ml/value_model.py` gained real persistence (same
schema-versioned/kind-tagged shape as every prior Tier 6 model) and a
real call site — `_voice_narrative_extra_scores` (the weekly voice-
pair "protagonist" score) folds in a loaded model's predicted
consequence as a bounded additive bonus, alongside the existing hand-
set inventor/council/Humans-pillar bonuses; `None` reproduces the
exact prior behavior byte-for-byte. New `scripts/train_value_model_
from_archive.py`: one example per living core-cast agent, features =
...
per agent, not a true series, since `Observation` carries no
`agent_id` to reconstruct a real per-observation pair from.

L4.1: `hearthmind/ml/belief_calibration.py` gained the same
persistence shape. New shared `SimulationEngine._calibrated_
confidence` helper wired at `_maybe_schedule_self_tuning`'s C2
conviction gate — the one place a still-open hypothesis's raw stated
confidence decides whether it's trustworthy enough to initiate a real
sandboxed self-tuning attempt. New `scripts/train_belief_calibrator_
from_archive.py`: trains directly from `World.reflection_notebook`'s
own real settled outcomes, no recorder archive needed.

New `scripts/train_all.py`: one command runs every local trainer this
project ships (goal policy, embedding, value model, belief
calibrator, the four decision policies, the law scorer, the LLM cost
regressor) against one world's db + recorder archive, writes every
weights file into the same directory, and reports what trained vs.
what was honestly skipped for lack of data.

README's "Local ML training" section substantially extended: both new
models added to the model table; a new "Training everything in one
command" section; a new "How long should I wait before training, and
do I need to stop the world?" section (training is read-only/offline,
safe against a live world; per-model wait times stated honestly and
separately — embedding/value model minutes-to-an-hour, belief
calibrator real days since Reflection settles slowly by design, the
five archive-backed models hours-to-weeks depending on how often
their real trigger fires); the "Automating it" section extended to
note the workload forecaster's own L5+L6 (v1.34.264) already run
automatically in-engine; new "A different shape" write-ups for both
models.

New `scripts/verify_l2_1_l4_1_wiring.py` (23 checks, all pass —
persistence round-trip/schema/kind rejection, the real engine-loading
cases, `full_diagnostics()` surfacing, `_calibrated_confidence`'s
identity-vs-real-calibrator cases incl. a live proof it can flip the
self-tuning conviction gate's own threshold check, no-op/live-bonus
proofs for `_voice_narrative_extra_scores`).

Verified: the new script (23 checks); `scripts/verify_l6_workload_
genome_evolution.py`/`scripts/verify_l5_goal_policy_retrain.py`/
`scripts/verify_llm_cost_regressor_wiring.py` re-run clean; `pyflakes`
clean on all touched/new files (only the six known pre-existing
forward-ref findings in `engine.py`); a real end-to-end smoke test of
`train_all.py` against a genuine ticked world confirmed the value
model trains and its weights load back through the engine, and the
db-only-vs-archive-needed skip logic degrades correctly; `scripts/
verify_replay_hash.py` (800 ticks, seed 777, `--in-process`) — MATCH,
byte-identical; `scripts/verify_native_soak.py` (seeds 1/55, 800
ticks) — MATCH.

**This closes roadmap Phase 2 in full** — all six named items (L2.1,
L3.1, L3.2, L4.1, L5, L6) now shipped, wired, and locally trainable.

## Current state (v1.34.264)

Explicit user instruction: "continue with roadmap phase 2" — closes
L6 (`hearthmind/ml/evolution.py`'s population-level evolutionary
substrate, G4-proven since v1.34.176 but never given a real live
consumer). Wired to the same `WorkloadForecaster` G2/L5 already
retrain monthly.

New `SimulationEngine._maybe_evolve_workload_genomes` (yearly,
`_TICK_JOBS`, placed BEFORE `_maybe_tick_workload_forecaster` — a
real year_end tick always also fires month_end/season_end/day_end the
same tick, confirmed against `time_system.py`'s own boundary logic, so
this safely reads `_workload_training_examples` first). Scores a real,
small (`WORKLOAD_GENOME_POPULATION_SIZE=6`, `_MU=3`) `GenomePopulation`
via `evaluate_and_select` against the same examples the monthly
retrain already consumes; the population's best genome is compared —
same cycle, same `fitness_fn` — against a throwaway `ModelGenome(
genome_id="__live__", ...)` built from whatever's currently live,
never appended to the population itself (its configured size never
changes across generations). Only a genuine improvement updates
`self._workload_learning_rate_override`/`_workload_epochs_override`;
`_maybe_tick_workload_forecaster`'s `learn()` call now reads both
instead of the flat `WORKLOAD_LEARNING_RATE` constant + an implicit
`epochs=20` — a world that never adopts a fitter genome keeps
byte-identical monthly-retrain behavior indefinitely. Every real
generation (adopted or not) logs to a new bounded `_workload_genome_
evolve_log` (`WORKLOAD_GENOME_EVOLVE_LOG_MAX=24`), surfaced via
`full_diagnostics()['workload_forecaster']['genome_population']`.

This is L6's own real distinction from L5 made concrete: L5 is one
lineage continually learning (ontogeny — warm-start + replay,
already wired for both `WorkloadForecaster` and `GoalPolicy`); L6 is
variation/inheritance/selection across candidate hyperparameter
CONFIGURATIONS (phylogeny) — the same `fitness_history`/`generation`/
mutate-and-select shape `world/ontology.py`'s `InventedConcept` and
`agents/population.py`'s diploid inheritance already use, applied to a
learned model's own hyperparameters for the first time.

New `scripts/verify_l6_workload_genome_evolution.py` (14 checks, all
pass — population seeding, no-op below threshold/off year_end, a real
generation firing and logging exactly once, training examples staying
uncleared (only the monthly retrain owns that), population size
invariant across many real generations, a genuinely fitter evolved
genome adopted over a deliberately-worse live baseline and a genuinely
worse one correctly NOT adopted — both via a deterministic monkeypatch
of `train_and_score_genome_via_specialist` at the `hearthmind.
simulation.engine` module level, isolating the adoption LOGIC from
real SGD training-dynamics noise (the first draft of both checks
relied on real training producing a large-enough gap on trivial
synthetic data and flaked); the monthly retrain genuinely reading the
live overrides (a real test-design bug caught and fixed: the first
attempt called `_maybe_tick_workload_forecaster(["month_end"])` alone,
missing that its own first line is `if "day_end" not in events:
return` — a real month boundary always also crosses a day boundary);
`_TICK_JOBS` ordering; `full_diagnostics()` shape; a short no-evolution
soak; a real end-to-end multi-year soak genuinely producing at least
one real evolutionary generation). The soak check drives a real
`Config(sim_minutes_per_tick=120)` engine instead of the default 15 —
confirmed harmless by direct measurement, since the check's own claim
depends only on how many CALENDAR units have elapsed (a pure function
of `sim_minutes_per_tick` against each unit's fixed real-minute
length), never on tick-to-simulated-time fidelity, and 120 needs 8x
fewer real `_tick_once()` calls to cross the same 370 simulated days
(4,440 vs. 35,520 — measured ~21s vs. 10+ minutes at the default).
...
tick-dispatched method all changed). Phase 2's other two items (L2.1,
L4.1) remain open — both genuinely need a live-deployment recorder
archive this offline environment has no way to produce — resume only
...
("Wire the already-built ML substrate to real consumers") names six
items; investigation found L3.2 (`WorkloadForecaster`) already fully
wired under HCA's G2 (v1.34.217) — a stale grouping in the roadmap's
own L3.1/L3.2 bullet — leaving L3.1 (`hearthmind/ml/llm_cost.py`) as
the real open item this pass closes.

`LLMCostRegressor.predict()`/`should_preflight_defer` were already
real, tested pure functions with zero persistence and zero real call
site. Gained schema-versioned/kind-tagged `to_dict`/`from_dict`/
`save`/`load` (same shape every other Tier 6 model this session
shipped). `SimulationEngine.__init__` loads an optional world-adjacent
`llm_cost_regressor_weights.json` (same file-next-to-`db_path`,
never-auto-created discipline as `GoalPolicy`/`DecisionPolicy`/
`LawCandidateScorer`); `None` (no file) means `_schedule_llm_job`
never builds a prediction or consults the regressor at all — verified
directly.

**Real wiring.** `_schedule_llm_job` now consults a loaded regressor
right after the daily-budget check, before a call is ever dispatched —
a call predicted unusually slow AND the queue already elevated
(`should_preflight_defer`) resolves synchronously to the deterministic
fallback (critical defers, non-critical applies inline) instead of
ever creating a background task or consuming a concurrency slot,
attacking `calls_dropped_backpressure` at its root rather than
reacting after dispatch. New shared `_resolve_llm_job_via_fallback`
factors the critical-vs-non-critical resolution out of both this new
gate and the pre-existing daily-budget-exhausted gate (previously
duplicated inline). Every real, non-fallback call's actual elapsed
time is scored against its own prediction via `CostPrediction
AccuracyTracker`, feeding the reliability weight `should_preflight_
defer` scales its decision by. `full_diagnostics()['llm_cost_
regressor']` surfaces `{loaded, path, accuracy: {mean_absolute_
error_ms, reliability_weight}}`.

**Real numerical-stability bug found and fixed before shipping.** A
direct sweep against data shaped like this project's own documented
real-world latency range (deep_reasoning outliers up to ~500000ms, per
this file's own prior diagnostic history) found the module's existing
`LATENCY_SCALE_MS=1000.0` reliably diverged `MLP` training to NaN/inf
at every learning rate tried — the same bug class v1.34.174/v1.34.257
already document for this codebase's other regressors, here
undiscovered until this pass's own wiring work exercised the model
against realistic data instead of the existing verify script's
narrower synthetic range. Raised to `100000.0` — the smallest value in
a `{1000, 10000, 60000, 100000} x {0.001, 0.0005, 0.0001}` sweep that
stayed finite and kept genuinely differentiating cheap vs. expensive
calls across 6 independent seeds. `scripts/verify_llm_cost.py`'s own
hardcoded assertion (written against the old scale) updated to match.

New `scripts/train_llm_cost_regressor_from_archive.py`: scans every
`LLM_COST_TASKS` archive subdirectory (this model's real consumer
applies across all of them, not one task), extracting only `fallback_
used=False` pairs with a real recorded `latency_ms`. Honest gap in its
own docstring: `current_backlog_fraction`/`concurrency_limit` aren't
recorded per-example anywhere in `llm/recorder.py`'s schema, so both
default to `0.0` for every training example — the model learns
...
New `scripts/verify_llm_cost_regressor_wiring.py` (19 checks, all pass
first run) — round-trip/schema-rejection/kind-rejection persistence; a
real training-convergence proof; a dedicated regression-proof test
training on realistic outlier-shaped data across 6 seeds confirming no
NaN/inf (the direct proof of the `LATENCY_SCALE_MS` fix); the three
real engine-loading cases; `full_diagnostics()` shape; a 3000-tick
no-regressor soak proving zero behavior change at the default; two
live end-to-end proofs directly against a real `_schedule_llm_job`
call — a constant-100000ms regressor under forced elevated backlog
resolves synchronously via the fallback with zero background tasks
created, a constant-100ms regressor dispatches normally (one
background task created); accuracy-tracker and `should_preflight_
defer` sanity checks; two subprocess end-to-end training-script runs
(a real success case, and a `fallback_used=True`-only archive
correctly training nothing and exiting 1).
...
(confirming this pass's `engine.py` edits didn't disturb either prior
session's wiring); `pyflakes` clean on all touched/new files (only the
six known pre-existing forward-ref findings in `engine.py`); `scripts/
verify_replay_hash.py` (800 ticks, seed 777, `--in-process`) — MATCH,
byte-identical; `scripts/verify_native_soak.py` (seeds 1/55, 800
ticks) — MATCH (both required — `simulation/engine.py`'s own
...
background audit's findings for all three large docs.

`docs/REFACTOR-2026-07.md`: 978 -> 194 lines. The R5-R8 native-C++-
port narrative (~24 modules' worth of "Module N shipped: here's what
was built and verified") compressed to a module-name -> `cpp/src/`
file list plus a "lessons that generalize" section keeping the real
reusable knowledge (STL casters copy not reference; identity-keyed
index iteration order leaks into tie-breaks; the real test for
whether a loop's RNG draws can be pre-drawn is same-pass dependency,
not draw-count; enums cross the pybind11 boundary as plain ints;
verify the wrapper function, not just the raw ported function). R1
(the `population.py` mixin split) and R3 (remaining `clamp()` sites)
— real open items — moved into the roadmap's Phase 8.

`docs/HEARTHBENCH-RUNTIME-2026-07-23.md`: 2425 -> 619 lines. Part B
(B0-B15, the Adaptive Runtime) held nearly all the bloat — each item
compressed to what it is + status + genuinely open sub-items. Part A
(HearthBench, still ~unbuilt) and Part C/SEQUENCE/TESTS/DECISIONS
RECORD untouched. One real staleness caught: this doc's own B8
section still says B8.1-B8.3 (the workload forecaster) is unwired —
it was in fact wired for real later under Tier 7 HCA's `G2`
(v1.34.217), a fact this doc never got updated to reflect. Also fixed
a literal duplicate A1.3 bullet. B15.6/B15.7/B15.8 (save-file profile
recording, scheduler fuzz testing, a tunable-registration-time safety
check) added to the roadmap's Phase 5.

`docs/MASTERCHECKLIST-2026-07-22.md`: left mostly intact (1383 ->
1400 — a status note added, not a rewrite) — the audit found this
doc's own Status/Spec/Data-model/Feeds structure is genuine, still-
needed spec (~45% redundant history vs. the other two docs' ~70-85%),
each item being only a first slice of a much larger named vision.
Fixed instead: five section headers (A5/A6/A7/A12/A13/A15) still said
`MISSING` after their own first bullet said "shipped a first slice"
— corrected to `PARTIAL`; A1's stale pre-shipment Status bullet marked
superseded rather than silently rewritten. The long un-rolled-up tail
of open sub-items (11 more `FieldGrid` fields; A10/A11/A13/A14/A15/
A16/A17/A18/A19/A20's named-but-unbuilt pieces; B5/B6/B8's remaining
gaps; C4's missing runtime auditor) folded into the roadmap's Phase
8/9 rather than left implicit in the roadmap's original 9-item
illustrative list.

`docs/ROADMAP-2026-07-REMAINING.md` gained ~20 bullets from both
audits (Phase 5: B15.6-8; Phase 8: R1, R3, and the MASTERCHECKLIST
tail above; Phase 9: C4) — 227 -> ~290 lines, still far below its
pre-consolidation 6,013. Nothing found by either audit pass was left
untracked.
...
directly following E1 (v1.34.242).
...
just has nothing to lose against yet. Closing this gap for real is the
newly-added roadmap Phase 8's own job; `describe_competition`/
`workspace_snapshot` make no assumption about coalition size, so they
need no change once a real multi-bid cycle exists.
...
re-run needed. `E3`-`E6` remain the open Stage E items — resume only
on future explicit direction naming a specific item.

## Current state (v1.34.242)

Explicit user instruction: "Start phase 7 E1" — HCA Stage E's first
item, directly following Stage D's close (D1 v1.34.240, D2 v1.34.241).

**E1.** The "why reasoning was or was not invoked" panel — this
file's own "Observatory UI direction" section had already named the
exact target line shape (`IMPASSE(no-change) · "family lines dying
out" · 590 occurrences, no rule · DELIBERATED (94s)` / `no impasse ·
peak surprise 0.3 < threshold 1.5 · cheap path`). New `hearthmind/
cognition/explain.py`'s `explain_cycle(impasse, outcome=None,
deliberation_seconds=None, no_impasse_detail="")` produces exactly
that line — pure presentation built entirely on C1's real `Impasse`
(kind/subject/detail, verbatim) and C3's real `DispatchOutcome`
(`resolved_via`), no new detection/dispatch mechanism. All three real
resolution tiers get their own honest label (`DELIBERATED (Ns)`/bare
`DELIBERATED` when no elapsed time is supplied/`CHUNK HIT (no
deliberation)`/`MODEL RESOLVED (no deliberation)`); an impasse with
no outcome yet correctly carries no fabricated resolution clause; a
no-impasse cycle accepts an optional caller-supplied `no_impasse_
detail`, since `detect_novelty` (and its siblings) return `None` with
no detail on a sub-threshold reading, and a caller wanting to show WHY
must pass the raw signal itself.
...
own filings — no replay-hash/native-soak re-run needed. `E2`-`E6`
remain the open Stage E items — resume only on future explicit
...
and parallely fix stale docs" — two independent pieces, one batch.
Tier 7 HCA Stage D's first item.

**D1.** New `hearthmind/cognition/activation.py`: the real ACT-R
base-level activation equation (`ln(Σ_j (t−t_j)^(−d))`, `d≈0.5`,
docs/COGNITIVE-ARCHITECTURE-2026-08-02.md §2.5) unifying recency AND
frequency into one quantity, plus spreading activation (salience/
relevance/causal-tag as three weighted associative sources) —
replacing `retrieve_relevant_memories`'s four hand-tuned `MEMORY_
RETRIEVAL_*` weights, all four genuinely **deleted** from
`hearthmind/agents/agent.py`, per D1's own stated test ("retrieval
quality holds... while four constants are deleted"). A real elapsed-
tick input needed a genuine new per-memory signal: new `Agent.memory_
ticks` (index-aligned with `memories`, same discipline as `memory_
causes`; a pre-D1 snapshot backfills via `age_ticks` + `LEGACY_MEMORY_
TICK_GAP`, an honest relative-order-preserving approximation, never
trusted as measured fact). Stamped by `_remember` from a new module-
level `population._CURRENT_TICK`, set once per real tick by
`Population.tick()`'s own top and mirrored by `SimulationEngine.
_tick_once()`'s own top for the handful of `_remember` call sites
living in engine.py's async LLM-job `apply()` closures — deliberately
NOT a `tick` parameter threaded through `_remember`'s own ~46 real
call sites, since this exact module already carries the direct
precedent for this shape (`_pending_memory_evictions`'s own docstring:
"`_remember` receives only `agent`, no reference back to Population/
World.clock").
...
(B8's reinforce counters), never threaded onto `Agent` — so every
memory here has exactly ONE real presentation, its formation tick;
...
no signature change here. No RNG anywhere in the scoring, matching
this project's own standing arbitration discipline (`hearthmind.
cognition.workspace`'s B2 redesign). `retrieve_relevant_memories`
gained an optional `current_tick` param, threaded through its one real
engine-side call site (`_run_personal_belief`, `self.world.clock.
tick_count`); its two `build_prompt`-side call sites (`llm/
cognition.py`, `llm/letters.py`) are deliberately left at the default
— both are pure functions with no engine access by design, degrading
honestly to the newest memory's own real formation tick as "now."

New `scripts/verify_d1_activation.py` (23 checks — the four deleted
constants confirmed genuinely gone via `hasattr`; the real zero-
division floor and same-tick edge case; a genuine frequency proof
(two presentations at the same age activate higher than one — a real
capability entirely absent from the deleted formula); recency still
dominating over stacked-but-old presentations at realistic decay; all
three spreading sources; end-to-end `_remember`/`Agent.memory_ticks`
production wiring; D1's own headline test — a real older high-
salience+relevant memory still outranks three merely-recent mundane
ones at a realistic elapsed-tick gap for a bounded memory list; the
`current_tick=None` production degrade; round-trip + legacy backfill
incl. preserved relative recency order) — all pass. One real test-
calibration fix made before shipping, not a bug in the module under
test: the first attempt used an unrealistic multi-year elapsed-tick
gap, which correctly LOST under a faithful single-presentation ACT-R
decay (honest model behavior, not a bug) — recalibrated to a
realistic gap for a bounded, capped memory list instead of weakening
the formula to fit an unrealistic scenario.
...
unstarted to real shipped detail; re-swept for any stale Stage C
references left over from the prior C1/C2/C3 sequence — none found.
...
replay_hash.py` (800 ticks, seed 777, `--in-process`) — MATCH, byte-
identical; `scripts/verify_native_soak.py` (seeds 1/55, 800 ticks) —
MATCH (both required this pass — `Agent`'s own persisted schema and
`simulation/engine.py` both changed). `D2` (declarative/procedural
separation made architectural) is the only remaining open Stage D
item — resume only on future explicit direction.
...
two independent pieces, one batch. **Tier 7 HCA Stage A's first item.**

**A1.** `predict()`/`error()` on specialists; precision-weighted
surprise scoring. New `hearthmind/cognition/surprise.py`'s
`SurpriseSpecialist`: one running forward model per string key
(Welford's online algorithm — O(1) per observation, no stored
history), implementing L1's `predict()`/`observe()`/`error()` trio per
the HCA doc's own formula, `surprise = |actual - predicted| / (sigma +
eps)`. Precision-weighting via the running standard deviation is what
lets a chronically noisy channel's typical deviation score LOWER than
the same raw gap on a tightly-clustered signal, and what lets a
recurring severe signal's own surprise genuinely fall once it stops
being novel. `bid()`/`learn()` deliberately out of scope — `bid()`
needs Stage B's real workspace (unbuilt), and a trained `learn()`
...
this offline environment to replay a real soak's event stream against:
a routine, near-identical repeated signal ["content agent socialises"]
scores < 0.1; a rare, severe, first-occurrence signal ["family
extinction during prosperity"] scores > 2.0; the formula matches the
doc's own math by hand; precision-weighting genuinely discounts a
chronically noisy channel against a tightly-clustered one; a recurring
severe signal's own surprise falls once genuinely learned; `score()`
is atomically equivalent to `error()`-then-`observe()`; calling
`observe()` before `error()` for the same value silently understates
surprise, demonstrating why the doc's own ordering note is load-
bearing) — all pass, first run, no bug found. Deliberately NOT wired
into any real production job this pass — `world/emergence.py`'s own
consumer (gating the emergence log on surprise instead of occurrence)
is A2, a distinct, larger item depending on this primitive existing
first, same "ship the interface, wire the first real consumer next"
discipline every prior Stage A/G/L item here has used.

**C++ porting backlog, parallel track.** New `cpp/src/terrain_
neighbor_count.cpp`: `world/terrain_evolution.py`'s `_tick_fallow`,
the weekly full-grid pass gating reforest eligibility — for every
...
`scripts/verify_replay_hash.py` (4000 ticks, seed 777, `--in-process`)
— MATCH, byte-identical (load-bearing this time — this pass touches
`world/terrain_evolution.py`'s real production tick path); `scripts/
verify_native_soak.py` (3 seeds x 3000 ticks) — MATCH. A1 touched no
native module or `simulation/engine.py` code path (pure offline
substrate); the `terrain_neighbor_count` port is the only native-
module change this pass.

## Current state (v1.34.219)
...
first run, no bug found. New toggle entries in `scripts/verify_
native_soak.py`.
...
## Current state (v1.34.218)

Explicit user instruction: "Start G4 and parallels c++ backlog" — two
independent pieces, one batch.

**G4.** `hearthmind/ml/evolution.py`'s `GenomePopulation` previously
scored every genome through `train_and_score_genome`, a bare `train_
mlp_sgd` call — a parallel evaluation path that never touched G1's
shipped `LearningSpecialist.learn()`. New `train_and_score_genome_
via_specialist`: builds a genome-shaped model, wraps it in a real
`LearningSpecialist`, and scores the genome through the exact same
`learn()` → shadow-gate cycle a live specialist would use —
`replay_fraction`/`epochs`/`learning_rate` genes map directly onto
`learn()`'s own keyword arguments (scoped 1:1 against a real `learn()`
call from the start, not invented fresh here). Fitness is read from
the real `candidate_metric` regardless of whether the shadow gate
accepted or rejected the candidate — a genome is scored on how well
its hyperparameters actually trained, not on whether that training
happened to survive gating this one time — with the same NaN-guard
discipline `train_and_score_genome` already established.

New `scripts/verify_ml_g4_genome_evolution.py` (9 checks — G4's own
stated test: a real `GenomePopulation`'s mean fitness climbs
monotonically across 8 real generations via `evaluate_and_select`
against the new "workload_forecaster"-species L1 consumer above; the
genome-scoring path reproduces an identical hand-driven `Learning
Specialist.learn` outcome; a deliberately-sabotaged retrain is
genuinely rejected by the real shadow gate with the live model's
weights byte-identical before/after; a genuinely NaN-diverged genome
degrades to fitness 0.0 rather than crashing or fabricating a score;
every surviving/bred genome's genes stay within `GENOME_
HYPERPARAMETER_BOUNDS`; the pre-existing `train_and_score_genome` path
is unaffected) — all pass. One real test-calibration fix made before
shipping, not a bug in the module under test: the first sabotage
scenario used a target extreme enough to NaN-diverge the candidate's
loss outright, making `candidate_metric > 0.0` an unreliable assertion
(`nan > 0.0` is `False`) — split into two real, separately-meaningful
checks: a shadow-gate-rejection proof (a moderate wrong target that
regresses without diverging) and a NaN-guard proof (the module's own
already-documented degrade-to-0.0 behavior, exercised directly with an
intentionally diverging target).

**C++ porting backlog, parallel track.** New `cpp/src/hydrology_
tick.cpp`: the full-grid scalar passes behind `world/hydrology_
field.py`'s `tick_hydrology`/`tick_groundwater` (A11, live in
production since v1.13.0), ported per that module's own docstring
explicitly inviting the port "once the shape is confirmed live,
following soil_fertility.cpp's precedent." Same "resolve enums/
objects in Python, hand C++ only plain data" discipline `climate_
drift.cpp` established: the caller precomputes a water-biome 0/1 grid
and a plain elevation grid before calling in, so the native file never
touches a `Tile`/`Biome` object. Deliberately scoped to just these two
functions: `tick_snowpack` depends on the still-unported `ca_
operators.reaction_diffuse`, and `tick_erosion` writes real `Tile`
objects with genuine biome reclassification (including the QUARRY-
sticky special case) — both a materially different, larger risk
surface than a pure scalar-grid pass, flagged as separate future work
rather than rushed into the same pass. `hydrology_field.py` branches
to the native path when available; `field.moisture`/`field.
groundwater` are reassigned wholesale (same pattern `tick_snowpack`'s
existing `reaction_diffuse` call already uses), not mutated in place.

New `scripts/verify_hydrology_tick_native.py` (5 checks — 2000-trial
randomized native-vs-Python-reference equivalence for each ported
function, plus empty-grid/all-water/single-tile edge cases) — all
pass, first run, no bug found. New toggle entries in `scripts/verify_
native_soak.py` for both functions.
...
engine.py` code path (pure offline ML substrate, no replay-hash/
native-soak re-run needed for that half specifically) — the hydrology
port is the only native-module change this pass, and both checks
above cover it.
