...
from hearthmind.agents._population_pathfinding import (
    PathfindingMixin,
...
mixin slice -- walkability/movement/target-search. `PathfindingMixin`
becomes a real base class of `Population`, below; the free functions
and `_NEIGHBOR_OFFSETS` are re-imported here so every bare-name
reference elsewhere in this file (and every external `from hearthmind.
agents.population import ...`, since this module re-exports
transparently) keeps working unchanged. See that module's own docstring
for the full design."""
...
@dataclass
class Population(PathfindingMixin):
