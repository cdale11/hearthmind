...
class TestPopulationTick(unittest.TestCase):
    ...
    def test_hunger_increases_each_tick(self):
        ...
        self.population.tick(seed=42, tick=1, terrain=self.terrain, resources=self.resources, settlement=self.settlement, farms=self.farms)
    ...
    def test_energy_drains_while_awake_and_triggers_rest(self):
        ...
        for tick in range(1, 60):
            self.population.tick(seed=42, tick=tick, terrain=self.terrain, resources=self.resources, settlement=self.settlement, farms=self.farms)
    ...
    def test_tick_deterministic_for_same_seed_and_tick(self):
        ...
        for tick in range(1, 20):
            pop_a.tick(seed=42, tick=tick, terrain=terrain, resources=resources_a, settlement=settlement_a, farms=farms_a)
            pop_b.tick(seed=42, tick=tick, terrain=terrain, resources=resources_b, settlement=settlement_b, farms=farms_b)
...
class TestDeath(unittest.TestCase):
    def test_sustained_starvation_kills_agent(self):
        ...
        for tick in range(1, STARVATION_TICKS_TO_DEATH + 5):
            events.extend(population.tick(seed=42, tick=tick, terrain=terrain, resources=resources, settlement=settlement, farms=farms))
