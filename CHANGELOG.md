...
real bench daemon, per A1.3's own framing: "The UI page talks to a
bench daemon, not the sim engine." **This closes step 6 of the
checklist's own SEQUENCE in full** — A12.6-A12.9 (compare/drill-in/
download/human-rating) remain real, distinct, unstarted future work
within the item itself, not blockers on step 6 closing.
...
...`), the one module importing `uvicorn` directly so `create_app`
itself stays importable/testable with no real ASGI server installed.
...
wiring `LlamaCppAdapter`/`OllamaAdapter` through needs the CLI itself
extended first, real symmetric future work); `GET /api/runs/{id}/
progress` + the page's own polling refresh (A12.3, real live case-count
progress — a genuine push/log-stream and live mid-run category scores
are honestly NOT shipped, flagged rather than faked); `POST /api/runs/
{id}/cancel` (A12.4, a real `BenchRunProcess.stop()` — only for a run
THIS daemon process itself launched; resume and cross-process cancel
need a real PID-file/lock mechanism this pass didn't build); `GET
/api/runs` (A12.5 — discovers EVERY real run under `runs_root` by
scanning for a real `manifest.json`, A8.2, never an in-memory registry,
so a restarted daemon can browse every past run with nothing to
rebuild); `GET /api/runs/{id}/report` (a real A9 HTML report, reusing
`recompute_run_metrics`/`compute_score`/`render_html_report` directly
— zero new scoring/reporting logic).
...
already pins, kept genuinely SEPARATE so neither install path pulls in
the other's dependency by accident.

New `scripts/verify_a12_bench_daemon.py` (24 checks, all pass — one
real test-script bug caught and fixed before shipping, not a bug in
the daemon: the first progress-polling loop didn't guard against the
real, brief startup race where a just-launched subprocess hasn't
created its own `run_dir`/`manifest.json` yet, so the daemon's own
honest 404 was misread as a progress payload — fixed to keep polling
on a non-200 response): the A1.2 firewall confirmed directly over the
whole daemon package; a REAL `uvicorn.Server` run in a background
thread, talked to via real `urllib.request` calls (never `TestClient`/
mocked) against a real local fake-backend HTTP server standing in for
a live model — the page's own banner text; the full real start→poll→
complete→report→cancel lifecycle for a genuine 4-case grounding run;
404s for an unknown run's progress/cancel/report; a genuine cancel-
while-running proof (a real slow-backend run, a real mid-run "still
...
daemon instance against the SAME `runs_root` correctly discovers a run
it never launched (`tracked_by_this_daemon: false`) purely from disk,
and correctly 404s a cancel attempt against it rather than fabricating
success.
...
`simulation/engine.py` code path touched — `git status` confirmed only
`hearthbench/runner/cli.py`, `pyproject.toml`, the new `hearthbench/
daemon/` package, and the new verify script changed — no replay-hash/
native-soak re-run needed. Per SEQUENCE, step 7 (`A4.2`'s remaining
subjective-category content authoring + `A4.3`/`A12.9`'s human-rating
page) is next — resume only on future explicit direction.

## [1.34.284] — Roadmap Phase 6 continues: HearthBench C5, the model passport

Explicit user instruction: "continue phase 6." Ships C5 in full — both
halves the checklist's own text names: real emission on the
`hearthbench` side, real runtime consumption on the `hearthmind` side,
coupled only by a shared JSON shape (A1.2's import firewall runs BOTH
directions — `hearthbench` never imports `hearthmind.simulation`/
`.agents`/`.world`, and `hearthmind` never imports `hearthbench` at
all, confirmed directly via AST in the new verify script rather than
assumed).
...
Score` + a real A2.1 adapter `describe()` (as a plain dict) + a real
`summarize_latency` output into a `ModelPassport` — model id/
quantization/file hash; the composite score + confidence margin
(`world_score` honestly `None`, since A5.11 doesn't exist yet);
category strengths/weaknesses (real score thresholds); measured
throughput (flat real values, never fabricated when unmeasured);
`recommended_settings.needs_grammar_constraints` (derived from the
real structured_outputs score); real hard warnings, one per A10.2
disqualification that actually fired. `peak_rss_mb_by_concurrency`
ships honestly empty — no A7.2 concurrency-sweep sampling mechanism
exists yet. `save_passport`/`load_passport` round-trip a versioned
JSON file.

New consumption on the `hearthmind` side (`simulation/hardware_
profile.py`): `passport_filename_for` (a deterministic filesystem-safe
slug of a model id, no registry file to keep in sync), `load_passport_
dict` (reads the passport's own JSON shape as a plain `dict`, degrades
to `None` on any failure — a bad/absent passport must never crash
startup), and `seed_machine_profile_from_passport` — "Passport values
are *priors*, not overrides." Seeds `MachineProfile.measured_llm_
throughput_tokens_per_s` ONLY while that profile has never had a real
live measurement of its own; deliberately NOT gated on session count
...
second-ever startup with zero live data having ever landed) — a
still-unseeded profile is re-consulted fresh every startup, so a newer
benchmark passport can update a stale prior at any time. `Machine
Profile` gained `passport_model_id`/`passport_warnings` (round-trip
safe, legacy pre-C5 profiles backfill to `None`/`[]`).

Wired at `SimulationEngine.__init__`'s real `_load_or_create_machine_
profile` call site via a new `_passport_path_for` (a `passports/`
directory sibling to wherever the world's own `MachineProfile`
persists, `None` under the identical `:memory:` condition its sibling
already exempts) — an operator who benchmarks a model with HearthBench
and drops the resulting `passport.json` at that exact path gets it
picked up automatically on the next startup, no config change needed.
The safety interlock ("a passport hard warning is surfaced in the UI
at startup"): `full_diagnostics()['machine_profile']` gained
`passport_model_id`/`passport_warnings`, rendered as a real "Model
passport: ⚠ ..." line in the existing "Adaptive runtime" dev-console
panel (`renderAdaptiveRuntimeStatus`, `app.js`) — pure presentation
over already-real data.

**Honestly flagged scope trim, not silently dropped**: the checklist's
own "a passport from a very different machine contributes throughput
priors with lower weight" is NOT implemented — `seed_machine_profile_
from_passport` seeds unconditionally regardless of host match. Real,
distinct future work needing a genuine similarity metric between two
hardware profiles this pass didn't design.

New `scripts/verify_c5_model_passport.py` (41 checks, all pass — one
real test-script bug caught before shipping, not a bug in either
module: the first end-to-end pass built a passport without threading
`latency_stats` into `build_passport` itself, so throughput stayed
honestly empty and seeding correctly found nothing to seed — fixed by
passing `latency_stats` to the builder, matching the real call shape
every other check already used): the A1.2 firewall confirmed directly
in both directions; every `build_passport` field incl. a real
disqualified run producing a real worded hard warning; round-trip +
schema-version rejection; a real cross-implementation parity proof
...
many-times-loaded-but-never-measured profile is STILL seedable — the
direct proof the session-count gate was correctly left out);
...
through `full_diagnostics()`, a mismatched model id neither seeds nor
crashes, and a `:memory:` db never touches disk for the lookup.
...
--check` clean on `app.js`. Unlike every other Phase 6 pass this
session, this ONE touches real `simulation/engine.py` code (the
...
with zero effect on deterministic Body state or RNG consumption. Per
SEQUENCE, `A12` (the web UI) is now the ONLY item left in step 6 —
...
HEARTHBENCH-RUNTIME-2026-07-23.md`'s own SEQUENCE section found a real
self-correction needed first: that section places "A13 CI regression
guard" right after "A4.1 deterministic scorers + A5.7/A5.8" — the
exact two things A4/A5 (v1.34.278-.279) just shipped — explicitly
BEFORE A6-A12. This doc's own `docs/ROADMAP-2026-07-REMAINING.md` had
instead deferred A13 to the very end ("needs the whole pipeline
first"), a mistake from an earlier pass, not the checklist's own
intent. Corrected in this pass rather than left to compound, and A13
ships now, ahead of A6-A12.

A13 needs SOME way to actually run a `TestCase` against a real
adapter and produce real per-category metrics — none of A11's named
run modes (quick/full/custom/resume/strict-repro) exist yet, so this
pass ships the one real execution-core slice A13 genuinely depends on,
not the full mode abstraction (flagged, explicitly, as still open).

New `hearthbench/runner/run.py`: `render_case_prompt(case, fixtures_
by_id)` resolves a `TestCase` into an executable prompt two ways — a
`turns`-carrying case renders through A3.3's existing `render_turn_
sequence` and sends the LAST rendered turn's content as a single-shot
prompt (real multi-turn conversation-state injection into a chat
history is a deliberate, flagged scope trim, not attempted); a
`fixture_ref`-carrying case resolves against a caller-supplied `dict`
of A3.1 `FixtureExample`s. Neither path fabricates a prompt — a case
matching neither shape returns `None`, and `run_case_against_adapter`
treats that as a genuine skip (no adapter call, no crash, no invented
result), never a silent zero. `run_case_against_adapter`/`run_cases_
against_adapter` are the real call-and-score loop over any duck-typed
adapter + `ScorerRegistry`; `aggregate_scores`/`summaries_to_metrics_
dict` group per-case `ScoreDetail`s by category+scorer via A5's own
`summarize_scores` and reshape into the plain nested dotted-path dict
`eval_harness.check_regressions` already consumes.

New `hearthbench/reporting/ci_guard.py`: A13.1 (`is_relevant_change`)
— real, maintenance-free trigger: any changed file under `hearthmind/
llm/` (every prompt builder + `json_schemas.py`) or `hearthbench/
prompts/`/`hearthbench/scoring/` (the shared cognition-contract
surface) counts, never a hand-maintained file list that silently goes
stale. A13.2 (`DEFAULT_CI_THRESHOLDS`) names only grounding/
structured-outputs metrics — deliberately nothing judge-scored, "too
noisy for CI" per the checklist's own text. A13.3 (`run_ci_guard`)
reuses `hearthmind.llm.eval_harness.check_regressions` DIRECTLY — no
second regression-comparison mechanism, real reuse of A0.2 exactly as
this item's own text instructs. A13.4 (`save_baseline`/`load_
baseline`) — a plain JSON file, refreshed only via an explicit call,
never implicitly by a regular guard run; a missing baseline degrades
to `{}` (nothing to regress against yet), matching `check_regressions`'
own "a metric missing from the candidate is a skip, not a violation."
A13.5 (a real nightly deeper CI run) stays explicitly unbuilt — needs
actual CI infrastructure this repo doesn't have, same honest gap as
every other "real CI wiring" item across Tier 5.

**Exercising the real end-to-end path for the first time found and
fixed a real, previously-shipped bug, not a new module's own bug.**
A5.7's `no_unsupported_specifics` scorer (`grounding.py`) was a real
`Scorer` object correctly listed in `GROUNDING_CATEGORY.scorer_ids`
and correctly declared on every grounding `TestCase`'s own `scorers`
field — but had never actually been `.register()`-ed into `hearthbench.
scoring.DEFAULT_REGISTRY`. `scripts/verify_a5_categories.py` never
caught this because it called the scorer function directly, bypassing
the registry entirely; this pass's new `registry.resolve(case.
scorers)` call path is the first code in the repo to ever look the id
up BY NAME, and `ScorerRegistry.resolve`'s own documented behavior is
to silently drop an unrecognized id rather than raise — so every
grounding case's real result set was silently missing this scorer's
score, invisibly, since A5.7 shipped. Fixed with one line inside
`hearthbench/tests/__init__.py` (the only legal place — `hearthbench.
scoring` already can't import FROM `hearthbench.tests`, since that
package imports the other way; extending the shared registry from the
category-owning package, one-directionally, is the correct fix):
`DEFAULT_REGISTRY.register(NO_UNSUPPORTED_SPECIFICS_SCORER)`. Re-ran
`scripts/verify_a5_categories.py` unmodified afterward — still 47/47,
confirming the fix only closed the registry-wiring gap and never
touched `grounding.py`'s own scoring logic.

New `scripts/verify_a13_ci_guard.py` (33 checks, all pass — one real
bug found and fixed as above, not a bug in the module under test):
`is_relevant_change`'s trigger logic; `render_case_prompt`'s three
resolution paths (turns, fixture, honest skip); `run_case_against_
adapter`/`run_cases_against_adapter` against a REAL local stdlib HTTP
server standing in for a model backend (same `_CapturingHandler`
technique `verify_a2_model_adapters.py`/`verify_a4_scoring.py` already
established) — including confirming a skipped case makes ZERO real
HTTP calls and that request counts match exactly; `aggregate_scores`/
`summaries_to_metrics_dict`'s real dotted-path output; the full `run_
ci_guard` path exercised three ways against three real fake-server
personas (a genuine no-op on an irrelevant change with zero server
calls; a real pass against a clean/hedging fake model; a real caught
violation against a fake model that confidently fabricates a
population count/name/founding year, with the violation naming the
real gated `no_unsupported_specifics` metric); `save_baseline`/`load_
baseline` round-trip incl. the missing-file-degrades-to-`{}` case; a
caller-supplied custom threshold dict genuinely overriding the default
(tested against a "about 300, give or take" marginal-fabrication case
that passes the default 0.80 floor but fails a stricter 0.99 custom
one); and final sanity checks confirming `DEFAULT_CI_THRESHOLDS` names
only objective metrics and every default CI case belongs to
`GROUNDING_CATEGORY`.

Verified: the new script (33 checks); `scripts/verify_hearthbench_
isolation.py`/`verify_hearthbench_adapter_isolation.py` both clean
(33 hearthbench files); `pyflakes` clean; `verify_a2_model_adapters.py`/
`verify_a3_prompt_library.py`/`verify_a4_scoring.py`/`verify_a5_
categories.py` all re-run clean, `verify_a5_categories.py` specifically
confirming the registry fix is additive-only. No native module,
`simulation/engine.py` code path, or other production file touched
(confirmed via `git status` — only the new `hearthbench/runner/run.py`/
`hearthbench/reporting/ci_guard.py`/`scripts/verify_a13_ci_guard.py`
plus three `__init__.py` re-exports/one registry-fix line) — no
replay-hash/native-soak re-run needed. A11's remaining real run modes
(quick/full/custom/resume/strict-repro) and A13.5's nightly-deeper-run
wiring are the next dependency-ordered items whenever resumed; per the
checklist's own SEQUENCE, A6 (structured-output validator) is the
next unstarted step in strict pipeline order.
...
found to be the flagship's own long-flagged gap: `GoalPolicy` (Tier 6
L2.2) was already built on G1's `LearningSpecialist` internally, but
nothing ever fed it a real live-cadence retrain the way G2 built for
`WorkloadForecaster` — v1.34.258/.259's own docstrings named this
exact item as "flagged as its own future item, not bolted on here."

`SimulationEngine._run_cognition` now captures a real `(agent_state,
goal)` pair straight from every genuine (non-fallback) LLM cognition
answer — encoded identically to `fallback_goal`'s own `goal_policy`
branch and to `scripts/train_goal_policy_from_archive.py`'s offline
extraction, just accumulated live instead of from a static export.
Bounded at `GOAL_POLICY_PENDING_EXAMPLES_MAX` (oldest evicted); only
appended when `self._goal_policy is not None`.

New `_maybe_tick_goal_policy` (monthly, `_TICK_JOBS`): once `GOAL_
POLICY_MIN_EXAMPLES_TO_RETRAIN` real examples have banked, splits off
a holdout slice and calls `GoalPolicy.learn` — already a thin
pass-through to its own wrapped `LearningSpecialist` (fixed to
`loss="cross_entropy"`), so no new training path was built. Reuses
the exact `lr=0.003`/`epochs=200` `scripts/train_goal_policy_from_
archive.py` already settled on for this model from a real archive
sweep (v1.34.258), rather than re-deriving. `self._goal_policy` is
mutated in place only on shadow-gate acceptance — no explicit
model-sync step needed, unlike G2's separately-held forecaster/
specialist pair. Deliberately does NOT write retrained weights back
to disk, same choice G2 already made — the in-memory policy keeps
improving across the running session; re-running the offline trainer
by hand against a fresher archive stays the supported path to a
durable weights-file update. `full_diagnostics()['goal_policy']`
gained `pending_examples_banked`/`learn_log_recent`.

New `scripts/verify_l5_goal_policy_retrain.py` (15 checks, all pass
first run) — capture-on-real-answer/no-capture-on-fallback/no-capture-
with-no-policy-loaded; correct field-by-field state encoding incl.
`materials_critical`/`has_plan`; bounded pending-example eviction;
the retrain gate's three real conditions (month_end, minimum
examples, non-empty after holdout split); a real accepted retrain
genuinely shifting the policy's prediction toward the taught goal
over several cycles; an all-malformed-goal batch degrading safely
without clearing pending state; `_TICK_JOBS` registration; `full_
diagnostics()` shape; a 400-tick no-policy soak proving zero behavior
change at the default; and a real end-to-end proof — a ~95-day soak
through the actual `_tick_once()`/background-task-draining path with
a real `GoalPolicy` and a fake LLM client produces a genuine retrain
attempt.
...
decision mechanism" — closes Phase 1's last flagged site, deliberately
excluded from v1.34.260's `DecisionPolicy` (a fixed-class shape that
can't represent a dynamic, ever-growing candidate set).

New `hearthmind/ml/law_scorer.py`'s `LawCandidateScorer`: the same
`RetrievalScorer` (L2.3) pattern — one scalar sigmoid model, never a
...
asked." The schema is deliberately generic (`occurrences_scaled`,
`pillar_confidence`, `initiated_by_conviction` — no candidate NAME
anywhere), so the same trained scorer applies to any of `_maybe_
schedule_laws`'s 14 named `pattern_key`s today, and to any future one
Tier 0 adds tomorrow, with no schema change and no retrain-from-
scratch — the genuinely different "ranking/scoring" shape the roadmap
flagged, as opposed to a fixed output-neuron-per-class classifier.
...
fabricates a law's TEXT, `forms`, or `kind` — `laws.fallback_laws()`
stays untouched, still the same honest "not yet" on every skipped/
...
schedule_laws`'s two real `max()` picks, appended after the real
occurrence count (primary, always dominant) and `village_pillar.
subject_confidence` (secondary, unchanged) — same "a lean only
breaks a genuine tie, the real signal is never overridden" discipline
every Tier 0 site already holds. New `SimulationEngine._law_candidate_
score` returns a constant `0.0` for every candidate when `self._law_
scorer is None` — since a constant never changes which key wins a
`max()`, this reproduces the exact prior two-key tiebreak byte-for-
byte, verified directly.

`simulation/engine.py` gained `LAW_SCORER_FILENAME`/`_law_scorer_
path_for`/`_load_law_scorer`, the same file-next-to-`db_path`, never-
auto-created pattern as every other Tier 6 model; `full_diagnostics()
['law_scorer']` surfaces `{loaded, path}`. `_maybe_schedule_laws`'s
real `_schedule_llm_job("laws", ...)` call site now also passes
`structured_input={occurrences, pillar_confidence, initiated_by_
conviction}` — previously nothing was recorded for this task at all,
so no archive could have trained this scorer before this change.
...
`rank()`/`learn()` correctness against a real synthetic pattern; the
engine's `_law_candidate_score` constant-0.0 no-op proof AND a real
trained-scorer tiebreak-flip proof (both directions, so the wiring is
provably live, not dead code); the three real loading cases (no file/
...
README's local-training guide extended: a new "A different shape: the
`laws.py` candidate scorer" section explaining why this needed a
...
The old "What's genuinely NOT available locally" section's `laws.py`
paragraph is removed — only real LoRA/QLoRA fine-tuning remains there
now.

Verified: the new script (19 checks); `pyflakes` clean on all touched/
new files (only the six known pre-existing forward-ref findings in
`engine.py`); `scripts/verify_decision_policies_wiring.py` (18 checks)
re-run clean, confirming this pass's engine.py edits didn't disturb
v1.34.260's wiring; `scripts/verify_replay_hash.py` (800 ticks, seed
777, `--in-process`) — MATCH, byte-identical, confirming the default
(`_law_scorer=None`) tick-loop behavior is completely unaffected.
`scripts/verify_native_soak.py` (seeds 1/55, 800 ticks) — MATCH.
...
step, wiring Stage C's chunk/dispatch ladder (C1's `Impasse`
classifiers, C2's `ChunkStore`, C3's `dispatch_impasse`) into a real
production `_schedule_llm_job` call site for the first time. Every
prior Stage C/E filing had honestly flagged this as unattempted — C1
through C3 shipped as real, tested, standalone primitives with no
live production consumer.

**Pilot.** `SimulationEngine._maybe_schedule_musing` (Reflection's
daily ambient-narration job) is now the first real job to consult a
genuine `Impasse`/`DispatchOutcome` pair every cycle. Chosen over
`_maybe_schedule_laws` (real Body-mechanical consequences, high
stakes) and `_maybe_schedule_rule_proposal` (season_end cadence, low
volume, touches the sandboxed trigger-rule path) for being low-risk
(pure ambient texture, zero mechanical consequence), high-volume
(daily), and because `_musing_subject()`'s own kind+text already
carries a genuine "same subject recurring" signal matching C1's
`detect_no_change` shape exactly — the HCA doc's own worked example.

New `SimulationEngine._musing_chunk_store` (a real `ChunkStore`,
runtime-only), `_musing_last_subject_key`/`_musing_no_change_streak`,
new constant `MUSING_NO_CHANGE_STREAK_THRESHOLD = 2`. Each real
`day_end` firing now classifies `detect_no_change` against the
subject's own real recurrence streak — below threshold dispatches
directly (byte-identical to the pre-Phase-8 path); at/above threshold
routes through `dispatch_impasse`, whose sole tier is a synchronous
closure that kicks off the real async `_schedule_llm_job` call exactly
as before and returns an honest `{"scheduled": bool, "tick": int}`
receipt as the "resolution" (never fabricated musing content — C3's
synchronous-return contract meets this codebase's real fire-and-forget
async LLM calls via an honest scheduling receipt, not a lie). A chunk
hit on a genuinely stagnant subject now skips the real call entirely.

New `scripts/verify_phase8_musing_pilot.py` (13 checks — C1's
threshold boundary; a real `SimulationEngine` driven day-by-day
through real `day_end` boundaries with a genuine static open
hypothesis, proving direct dispatch below threshold, the real first
LLM-tier dispatch/chunk-compile at threshold, a real chunk hit with no
new musing on a stagnant subject, and a real subject change resetting
the streak; C3's own stated live-soak bar — a real 40-day soak on a
static subject resolved 92.5% of real opportunities without reaching
the LLM, well past the stated >30% bar) — all pass. One real test-
harness bug caught and fixed before shipping, not a bug in production:
the first draft drove `_maybe_schedule_musing` directly, bypassing the
real `_tick_once()` tick loop, so `_reserved_this_tick` (reset only at
the top of a real tick) accumulated across simulated days instead of
resetting, tripping a false-positive backpressure block from day 3
onward — fixed in the test harness, not the production code.

Verified: the new script (13 checks); `pyflakes` clean (only the six
known pre-existing forward-ref findings in `engine.py`); `verify_c1_
impasse.py`/`verify_c2_chunking.py`/`verify_c3_dispatch.py`/`verify_
runtime_invariant.py`/`verify_d2_memory_kind.py` re-run clean
(unaffected); `scripts/verify_replay_hash.py` (800 ticks, seed 777,
`--in-process`) — MATCH, byte-identical; `scripts/verify_native_
soak.py` (seeds 1/55, 800 ticks) — MATCH. `E1 goes live` and `Sweep`
(Phase 8's remaining two steps) stay open — resume only on future
explicit direction.
