...
farming, wildlife/ecology, roads, a live browser UI with a dev
...
- Vehicles (hauling + personal travel), terrain evolution (both local
  activity-driven change and longer-term climate/biome drift), and a
  richer browser visual layer (weather particle effects) — all
  requested, not yet built; see chat for scoping notes until a
  DECISIONS entry exists.
